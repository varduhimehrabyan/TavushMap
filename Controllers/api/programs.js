...
router.post('/programs', tokenVerify, async (req, res) => {
  try {
    ...
    const data = await pool.query(pgFunctions.programs.usp_programList, [null, language, null, null, null])
    res.send({
      data: data.rows
    })
  }
...
router.put('/editProgram', tokenVerify, async (req, res) => {
  try {
    ...
    const { id, support, community, organization, programName_arm,
      ...
      contact_arm, contact_eng, isDonor, description_arm, description_eng, statusId } = req.body.prog
